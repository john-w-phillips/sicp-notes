...
(define (analyze-and exp)
  (define (analyze-and-ops ops)
    (let ((analyzed-ops (map analyze ops)))
      (lambda (env succeed fail)
	(define (iter ops)
	  (if (null? ops)
	      (succeed true fail)
		 ...
		 (if (falsy? pvalue)
	       ...
	       fail)))
	(iter analyzed-ops))))
  (analyze-and-ops (operands exp)))

(define (analyze-or exp)
  (define (analyze-or-ops ops)
    (let ((analyzed-ops (map analyze ops)))
      (lambda (env succeed fail)
	(define (iter ops)
	  (if (null? ops)
...
(define the-global-environment
  (extend-environment
   ...
   (map (lambda (x) (make-primitive-procedure (cadr x))) primitive-procedures)
   the-global-environment))
